...
Rename and the "Add heading" quick fix return document edits: a file that is open carries the buffer version the edits were computed for, so the client refuses them if you typed in the meantime; files on disk carry no version.
