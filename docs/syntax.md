...
- `|Display Text` sets custom display text; it has no effect on resolution. Inside a table cell write the separator as `\|` (`[[note\|Display Text]]`); the backslash belongs to the separator, not to the target.
- A link that points at nothing (`[[]]`, `[[ ]]`, `[[|x]]`, `[[#]]`) is plain text, not a link.
...
Markdown-style links are also indexed. A relative path is read from the folder of the note that contains the link (`[t](../b.md)`, `[t](./c.md)`, `[t](d.md)`); a path that climbs out of the vault is broken, and when nothing is found there the vault-relative path and then the file name are tried. Wikilinks are not affected: they always use the vault-wide order below. Backlinks, orphan detection, broken-link counts, the graph, references and rename all use this same reading, so a link that is reported broken is never counted as a backlink. Percent-escapes in a note target are decoded (`[t](my%20note.md)` names `my note.md`; external URLs are left as written). If the destination isn't an external URL (`https://`, `mailto:`, `tel:`, `obsidian://`, `file://`, ... — a scheme followed by `//`, or one of `mailto`, `tel`, `sms`, `geo`, `data`, `javascript`, `urn`, `magnet`), it's resolved against the vault the same way a wikilink target is (including an optional `#Heading` suffix). External links are recognized but never checked for brokenness and never become graph edges. An external URL is kept whole (its `#fragment` is not a note heading). A note name may still contain a colon (`Project:Alpha`) and `C:\dir\x.md` is a path, not a URL.
...
- **A leading UTF-8 byte order mark is not content.** Files saved with a BOM parse exactly like the same text without one (the `---` fence is still found), and `satz fmt` / the formatter keep the mark on write.
- Recognized fields: `title` (or falls back to the first `# H1` heading, or falls back to the filename stem, or `"Untitled"`), `aliases` (or singular `alias`; both a YAML list and a single string are accepted), `tags` (or singular `tag`; a list, or a plain string of tags separated by commas and/or spaces — list items are never split, `tags: [a, "b c"]` gives `b c`; both `tags` and `tag` are read and merged, likewise `aliases`/`alias`; like a body tag a frontmatter tag needs at least one letter, so `2024` is dropped), `date`.
- Any other key is preserved verbatim in a passthrough map rather than causing an error.
- Malformed YAML never crashes the parser — the document just gets an empty frontmatter instead, and the language server reports it as an `invalid-frontmatter` warning on the block.
